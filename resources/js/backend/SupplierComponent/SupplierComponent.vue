<template>
  <div>
        ...
        <div class="card">
          <div class="card-body">
            <div style="min-height: 300px;">
              ...
              <form @submit.prevent="getListDataSuppliers()" class="row">
                <div class="mb-3 col-12 col-md-6 col-lg-3 col-xl-2">
                  <label class="form-label" for="nroDocumentSearch">RUC/DNI <i class="fas fa-question-circle" title="Presione la tecla ENTER para buscar"></i>
                  </label>
              ...
              <div v-if="listDataSuppliers.total > 0 && can(canView)" class="col-12">
                <div class="table-responsive">
                  <table class="table table-sm table-hover table-bordered">
                    ...
                    <tbody class="vertical-align-middle">
                      <tr v-for="(supplier,index) in listDataSuppliers.data" :key="index">
                        <td class="options" v-if="can(canEdit, canUpdateStatus)">
                          <button
                          v-if="can(canEdit)"
                          class="btn btn-sm btn-primary"
                          @click="openModalEditUser(supplier)">
                            <i class="far fa-edit"></i>
                          </button>
                          <button
                          v-if="can(canUpdateStatus)"
                          @click="updataStatus(supplier)"
                          :class="`btn btn-sm btn-${ supplier.is_active ? 'success' : 'danger' }`">
                            <i v-if="supplier.is_active" class="far fa-check-circle"></i>
                            <i v-else class="far fa-times-circle"></i>
                          </button>
                        </td>
                        <td class="status">
                          <span v-if="supplier.is_active" class="badge rounded-pill bg-success registration-status">Activo</span>
                          <span v-else class="badge rounded-pill bg-danger registration-status">Desactivado</span>
                        </td>
                        <td class="text-center cursor-default user-select-all"> <span class="cursor-pointer" @click="copiedAtClipboard(supplier.nroDocument)">{{ supplier.nroDocument }}</span></td>
                        <td>{{ supplier.fullName }}</td>
                        <td> <a :href="`mailto:${supplier.email}`">{{ supplier.email }}</a> </td>
                        <td class="text-center"> <a :href="`tel:${supplier.nroPhone}`">{{ supplier.nroPhone }}</a> </td>
                  ...
                  <pagination class="select-none"
                    :limit="5"
                    size="small"
                    :data="listDataSuppliers"
                    @pagination-change-page="getListDataSuppliers"
                  >
    ...
    <div class="modal fade" id="modal-supplier" tabindex="-1" aria-labelledby="Modal para registrar proveedores" aria-modal="true" role="dialog">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title select-none" id="Modal para registrar proveedores"> {{ modalTitle }} Proveedor </h5>
              ...
              <template v-if="typeDocument">
                <div class="mb-3 col-12 col-md-6 col-lg-4">
                  ...
                  <div class="input-group bootstrap-touchspin bootstrap-touchspin-injected">
                    <input type="text" v-model="nroDocument" class="form-control" placeholder="71560243" @keypress.enter="searchDocument">
...
<script src="./SupplierComponent.js"></script>
